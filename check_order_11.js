...
async function check() {
  console.log("Checking order PED-20260511-011 details...");
  const { data, error } = await supabase
    .from("pedidos")
    .select("numero_pedido, forma_pagamento, metodo_pagamento, pagamento_confirmado, status_pagamento")
    .eq("numero_pedido", "PED-20260511-011")
    .maybeSingle();
    
  if (error) {
    console.error("DEBUG_ERROR:", error);
  } else {
    console.log("ORDER_PAYMENT_DATA:", JSON.stringify(data, null, 2));
  }
}
