...
function ProdutoFormDialog({ mode, cats, onClose, onSaved }: { mode: Mode; cats: Categoria[]; onClose: () => void; onSaved: () => void }) {
  ...
  useEffect(() => {
    ...
    setForm(
      initial
        ? { ...initial }
        : { nome: "", descricao: "", preco: 0, quantidade_estoque: 0, ativo: true, destaque: false, categoria_id: cats[0]?.id ?? null, imagem_url: "" }
    );
  }, [open, mode]);
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <Input value={form.imagem_url ?? ""} disabled={isView} onChange={(e) => set("imagem_url", e.target.value)} />
          </div>
          <div className="space-y-2">
            <Label>Preço (R$)</Label>
          ...
          <div className="space-y-2">
            <Label>Estoque</Label>
            <Input type="number" value={form.quantidade_estoque ?? 0} disabled={isView} onChange={(e) => set("quantidade_estoque", e.target.value)} />
          </div>
          ...
          <div className="flex items-center gap-3">
            <Switch checked={!!form.destaque} disabled={isView} onCheckedChange={(v) => set("destaque", v)} />
